    ...
    while True:
        try:
            ...
            if op2 == 1:
                ...
                print("-" * 16)
                id_livro = consulta_id - 1               
                if id_livro < len(lista_livro):
                    for livro in lista_livro:
                        if livro['id'] == consulta_id: 
                            print(f'id: {livro["id"]}\nnome: {livro["nome"]}')
                            print(f'autor: {livro["autor"]}\neditora: {livro["editora"]}\n')
                else:
                    print('Desculpe, não há nenhum livro registrado com esse id.')  
            elif op2 == 3:
                ...
                for livro in lista_livro:
                    if livro['autor'] == consulta_autor:
                        ...
                        print(f'autor: {livro["autor"]}\neditora: {livro["editora"]}\n')  
                if conta_autor == 0:
                    print('Desculpe, não foi possível encontrar este autor.')   
            elif op2 == 4:
